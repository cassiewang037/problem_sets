...
```{r}
filtered_d <- subset(d, is.na(DoNotUse) | DoNotUse != 1)
...
rating_hyp_long_d <- rating_hyp_d |> 
    pivot_longer(cols = everything(), 
                 names_to = "Measurement", # Rename 'Key' column to 'Measurement'
                 values_to = "Rating")  
...
rating_summary_d <- rating_hyp_long_d |>
  filter(!is.na(Rating), !is.na(Emotion), !is.na(ConfrontationalGame)) |>
...
performance_summary_d <- performance_diff_d |>
  ...
  summarize(MeanImprovementScore = mean(PostMusic, na.rm = TRUE), .groups = "drop")
```
...
performance_summary_d <- performance_diff_d |>
  group_by(ConfrontationalGame, MusicCondition) |>  
  summarize(
    MeanImprovementScore = mean(PostMusic, na.rm = TRUE),  
    SEM = sd(PostMusic, na.rm = TRUE) / sqrt(n()),.groups = "drop"
  )
...
ggplot(performance_summary_d, aes(x=ConfrontationalGame, y=MeanImprovementScore, fill=MusicCondition)) + 
  ...
  scale_fill_brewer(palette="Set1") + 
  labs(x = "Confrontational Game", y = "Mean Improvement Score", fill = "Music Condition") + 
  theme_minimal()
```
